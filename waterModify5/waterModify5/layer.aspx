...
<body>
  ...
  <select  id="s1" name="oneclass" onchange="chooseLayer()">
    <option selected="selected" >按井盖模块类型选择</option>
    <option >只有报警功能</option>
    <option >报警及液位</option>
  </select>
...
<script type="text/javascript">
    ...
    var wellLayer = new Layer();//井盖图层类型
    var levelLayer = new Layer();//液位图层
    ...
    function addMarker() {
        ...
        for (var i = 0; i < jsonData.data.length; i++) {
            ...
            if (json.well_Type == "只报警") {
                wellLayer.data.push(marker);
            }
            else if (json.well_Type == "报警及液位") {
