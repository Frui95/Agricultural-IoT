...
#数据库文件，而是直接打开该数据库文件#!/usr/bin/env python
#coding:utf-8
...
class TerminalFrame(wx.Frame):
  ...
  def __init__(self, *args, **kwds):
    ...
    self.text_ctrl_output = wx.TextCtrl(selfwaiting, -1, "", style=wx.TE_MULTILINE|wx.TE_READONLY)
  ...
  def OnSerialRead(self, event):
    ...
    #text = ''.join([str(t) for t in list(bytearray(event.data)[:15])])
    data_list_wanted=[hex(x)for x in bytearray(event.data)[:7]]#二进制字节串的处理 python2.7只能这样做了
    airtemp=int(data_list_wanted[0],16)
    airhum=int(data_list_wanted[1],16)
    landhum=int(data_list_wanted[2],16)
    conduct=int(data_list_wanted[3],16)
    lux=str(int(data_list_wanted[4],16))+str(int(data_list_wanted[5],16))
    node_id=int(data_list_wanted[6],16)
    real_time=time.strftime("%Y-%m-%d %H:%M:%S",time.localtime())
    text="{node_id:"+str(node_id)+":\n\tdata:{airtemper:"+str(airtemp)+"℃,airhum:"+str(airhum)+"%,landhum:"+str(landhum)+"%,conduct:"+str(conduct)+",lux:"+lux+"lux,real_time:"+real_time+"}\n}"

    con=sqlite3.connect('/tmp/test.db')
    cursor=con.cursor()
    try:
      
      cursor.execute('select sql from sqlite_master where type="table" and tbl_name="weather_nodes"')
      value=cursor.fetchall()
      if(len(value)==0):
        cursor.execute('create table weather_nodes(row_id integer primary key autoincrement,node_id text not null,airtemper text ,airhum text ,landhum text,conduct text,lux text,real_time text);')
        con.commit()
      insertSQL="insert into weather_nodes(node_id,airtemper,airhum,landhum,conduct,lux,real_time)"
      insertSQL+="values("
      insertSQL+="'"+str(node_id)+"',"+"'"+str(airtemp)+"',"+"'"+str(airhum)+"',"+"'"+str(landhum)+"',"+"'"+str(conduct)+"',"+"'"+lux+"',"+"'"+real_time+"')"
      cursor.execute(insertSQL)
      con.commit()
      print (str(cursor.rowcount)+'row effected')
    except sqlite3.Error as sqlerr:
      print ('Sqlite Error:'+str(sqlerr))
    finally:
      con.commit()
      cursor.close()
      con.close()
  ...
  def ComPortThread(self):
    ...
    self.serial.flushInput()#empty the buff zone
    while self.alive.isSet():			   #loop while alive event is true
      text = self.serial.readline()		  #read one, with timout
      if text:							#check if not timeout
        n = self.serial.inWaiting()	 #look if there is more to read
        if n:
          text = text + self.serial.readline() #get it
